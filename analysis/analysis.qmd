...
Authority ratings are an ordinal 5-point Likert scale, so we re-fit the same fixed-effects structure as a cumulative-link mixed model to verify that the linear-model conclusions are not artifacts of treating the scale as continuous. Sum-to-zero contrasts are applied so coefficients are interpretable as main effects, matching the Type III ANOVA above.

```{r model-ordinal}
authority_df_ord <- authority_df %>%
  mutate(
    rating_ord = factor(authority_rating, levels = 1:5, ordered = TRUE),
    syn_level = factor(syn_level),
    sem_level = factor(sem_level),
    modal_pres = factor(modal_pres)
  )
contrasts(authority_df_ord$syn_level)  <- contr.sum(2)
contrasts(authority_df_ord$sem_level)  <- contr.sum(2)
contrasts(authority_df_ord$modal_pres) <- contr.sum(2)

model_3way_ord <- clmm(
  rating_ord ~ syn_level * sem_level * modal_pres +
    (1 | participant_file) + (1 | sent_id),
  data = authority_df_ord,
  link = "logit"
)

summary(model_3way_ord)
```

# Comprehension Analysis

## Comprehension accuracy by experimental condition

```{r comp-by-shall}
comp_by_shall <- comp_df %>%
  group_by(modal_pres) %>%
  summarise(
    N = n(),
    N_correct = sum(is_correct, na.rm = TRUE),
    Accuracy = mean(is_correct, na.rm = TRUE) * 100,
    .groups = "drop"
  )
knitr::kable(comp_by_shall, digits = 2,
             caption = "Comprehension accuracy by shall presence")

shall_tab <- with(comp_df, table(modal_pres, is_correct))
chisq.test(shall_tab)
```

```{r comp-main-effects-model}
comp_df_f <- comp_df %>%
  mutate(
    syn_level  = factor(syn_level),
    sem_level  = factor(sem_level),
...
contrasts(comp_df_f$syn_level)  <- contr.sum(2)
contrasts(comp_df_f$sem_level)  <- contr.sum(2)
contrasts(comp_df_f$modal_pres) <- contr.sum(2)

comp_model <- glmer(
  is_correct ~ syn_level * sem_level * modal_pres +
    (1 | participant_file) + (1 | sent_id),
  data    = comp_df_f,
  family  = binomial,
...
car::Anova(comp_model, type = 3)
```

## Trial-level effect of comprehension on authority ratings
...
cat(sprintf("r = %.2f, t(%d) = %.2f, p = %.3f\n",
            cor_test$estimate, cor_test$parameter, cor_test$statistic, cor_test$p.value))
```

## Within-between decomposition

To determine whether the comprehension-authority relationship is driven by between-person differences (stable individual differences) or within-person variation (trial-by-trial), we decompose the effect.

```{r within-between-decomposition}
...
summary(within_between_model)
```

The within-between decomposition reveals that the association is entirely between-person: the between-person effect is significant, but the within-person effect is not, indicating that the relationship reflects stable individual differences rather than trial-by-trial variation in understanding.
...
## Figure 2: Comprehension accuracy by condition

```{r figure-comprehension-conditions, fig.width=6.5, fig.height=4}
...
comp_summary <- comp_df %>%
  ...
  summarise(
    accuracy = mean(is_correct, na.rm = TRUE) * 100,
    n = n(),
    se = sqrt(accuracy * (100 - accuracy) / n),
    .groups = "drop"
  ) %>%
  mutate(
    Syntax = factor(ifelse(syn_level == "high", "High", "Low"), levels = c("High", "Low")),
...
label_df_comp <- comp_summary %>%
  ...
  mutate(
    x_mid = 1.5,
    label = as.character(Jargon),
    y_offset = ifelse(Jargon == "High jargon", 1.5, -1.5)
  )

# Plot
p_comp_conditions <- ggplot(comp_summary, aes(x = Syntax, y = accuracy, color = Jargon, group = Jargon)) +
  geom_line(linewidth = 1.2) +
  geom_point(size = 3.5) +
  geom_errorbar(aes(ymin = accuracy - 1.96*se, ymax = accuracy + 1.96*se),
                width = 0.08, linewidth = 0.8) +
  geom_label(data = label_df_comp,
             aes(x = x_mid, y = y_mid + y_offset, label = label, color = Jargon),
             size = 2.8, label.padding = unit(0.15, "lines"),
             linewidth = 0, fill = "white", alpha = 0.85,
             show.legend = FALSE) +
...
ggsave("../paper/figure_comprehension_conditions.pdf", p_comp_conditions, width = 6.5, height = 4)
ggsave("../paper/figure_comprehension_conditions.png", p_comp_conditions, width = 6.5, height = 4, dpi = 300)
```

## Figure 3: Comprehension and authority (participant-level)

```{r figure-comprehension-authority, fig.width=5, fig.height=4}
# Compute participant-level summary
participant_summary <- combined_df %>%
  group_by(participant_file) %>%
  summarise(
    comp_accuracy = mean(is_correct, na.rm = TRUE) * 100,
    mean_authority = mean(authority_rating, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  filter(comp_accuracy >= 60) %>%  # Exclude sparse low-accuracy participants

  mutate(log_comp = log10(comp_accuracy))

# Add jitter manually on log scale for visualization
set.seed(42)
participant_summary <- participant_summary %>%
  mutate(log_comp_jittered = log_comp + runif(n(), -0.015, 0.015))

# Jittered scatter with loess on log-scaled x-axis
p_comp <- ggplot(participant_summary, aes(x = 10^log_comp_jittered, y = mean_authority)) +
  geom_point(alpha = 0.5, size = 2) +
  geom_smooth(aes(x = comp_accuracy), method = "loess", se = TRUE,
              color = "black", linewidth = 0.8, span = 0.9) +
  scale_x_log10(breaks = c(60, 70, 80, 90, 100), limits = c(58, 105)) +
  scale_y_continuous(limits = c(1.5, 5.2), breaks = 2:5) +
  labs(
    x = "Comprehension accuracy (%)",
...
ggsave("../paper/figure_comprehension_authority.pdf", p_comp, width = 5, height = 4)
ggsave("../paper/figure_comprehension_authority.png", p_comp, width = 5, height = 4, dpi = 300)
```
